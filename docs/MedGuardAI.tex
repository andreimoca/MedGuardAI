...
%  Course: Large Language Models (FMI, Year 1, Semester 6)
...
\fancyhead[L]{\small\color{medslate}MedGuardAI \textemdash{} Project Documentation}
...
{\normalsize
\textbf{Course:} Large Language Models \textemdash{} Faculty of Mathematics and Computer Science\\[4pt]
...
with the biggest gains on the safety-critical categories \textemdash{} hallucination handling
(0\%\,$\to$\,77.8\%) and drug Q\&A (+15.6 pp).}

\vfill
{\small\color{medslate}Research prototype \textemdash{} not a substitute for professional medical advice.}
\clearpage
...
six clinical tools to call \textemdash{} drug-information lookup, dosage calculation, drug-interaction
check, allergy check, emergency triage, and a live FDA fallback \textemdash{} always grounded by
...
\begin{itemize}[leftmargin=1.4em,itemsep=2pt,topsep=3pt]
  \item \textbf{Data ingestion} of new datasets \textemdash{} an OpenFDA / DailyMed label pipeline plus MedQuAD (via Kaggle) for fine-tuning;
  \item \textbf{RAG} \textemdash{} ChromaDB vector store, sentence-transformer embeddings, MMR retrieval;
  \item \textbf{Agent architecture} \textemdash{} a LangGraph graph wired to six clinical tools;
  \item \textbf{Task-specific fine-tuning} \textemdash{} QLoRA SFT on a curated medical Q\&A and triage set;
  \item \textbf{RLHF} \textemdash{} DPO on preference data, including a live human-feedback loop;
  \item \textbf{Toxicity / hallucination handling} \textemdash{} input and output guards plus a groundedness check;
  \item \textbf{Evaluation} \textemdash{} a hand-curated held-out test set with a base-vs-fine-tuned comparison.
\end{itemize}
...
drug-label passages, the model answers \textemdash{} calling clinical tools as needed \textemdash{}
...
calls \textemdash{} each tool result is fed back into the conversation \textemdash{} until it produces
...
truth \textemdash{} the model is told to prefer it over its own parametric knowledge and to say so when
...
triage set, then a DPO pass on preference pairs. The dashed arrow is the human-feedback flywheel
\textemdash{} real downvotes in the UI become preference data for the next DPO round.}
\label{fig:train}
\end{figure}

\subsection{Phase 2 \textemdash{} Supervised fine-tuning}
...
\subsection{Phase 3 \textemdash{} DPO (RLHF)}
Starting from the SFT adapter, a Direct Preference Optimization pass teaches the model to prefer the
safe answer over a plausible near-miss. Preference pairs come from two real, reproducible sources
\textemdash{} no LLM is used to generate them:

\begin{itemize}[leftmargin=1.4em,itemsep=2pt,topsep=3pt]
  \item \textbf{Seeded safety hard-negatives:} deterministic \emph{(chosen, rejected)} pairs templated
        from the symptom themes, where the rejected answer breaks exactly one safety rule \textemdash{}
...
a small preference set \textemdash{} while $\beta=0.1$ keeps the policy anchored to the supervised
...
sentences); re-checks toxicity; runs a \textbf{groundedness check} \textemdash{} factual claims (drug
...
\textbf{Emergency short-circuit.} A \textsc{high}-tier presentation \textemdash{} crushing chest pain,
stroke signs, severe trouble breathing, suspected overdose, anaphylaxis, suicidal intent, uncontrolled
bleeding, a first seizure, fever in an infant under three months, a severe head injury \textemdash{} is
...
The model is scored on a held-out set of \textbf{106 hand-written cases} \textemdash{} deliberately not
LLM-generated, to avoid leakage \textemdash{} in four buckets: drug Q\&A (32), symptom triage (36),
...
present, forbidden phrases absent, correct triage tier. On top of that, DeepEval LLM-judge metrics
\textemdash{} Faithfulness, Answer Relevancy, Hallucination, and a custom ``Medical Safety'' rubric
\textemdash{} are computed with an independent judge model (never the model under test), and ROUGE /
...
\legend{Base \textemdash{} Gemma-3-4b,Fine-tuned \textemdash{} QLoRA SFT + DPO}
\end{axis}
\end{tikzpicture}
...
Adversarial      & 20  & 100\%\ \ \,(20/20)& 100\%\ \ \,(20/20)& \textemdash\\
...
fine-tuning \textemdash{} that line is held by the input and output guards rather than by the model
\textemdash{} and it stays there. The DeepEval LLM-judge scores move in the same direction
...
\begin{itemize}[leftmargin=1.4em,itemsep=3pt,topsep=3pt]
  ...
  \item \textbf{Backend:} \texttt{cd backend}; \texttt{pip install -r requirements.txt};
        \texttt{PYTHONPATH=src python src/api/main.py} \textemdash{} serves on \texttt{localhost:8000}.
  \item \textbf{Frontend:} \texttt{cd frontend}; \texttt{npm install}; \texttt{npm run dev}
        \textemdash{} serves on \texttt{localhost:5173}.
  \item \textbf{Tests:} \texttt{cd backend}; \texttt{pytest} \textemdash{} covers the guards, tool
...
{\small\color{medslate}MedGuardAI \textemdash{} Borodi Bogdan-Vasile, Moca Andrei-Catalin, Petrea
Alexandra, Pauliuc C\u{a}lin George, Vaicum Ana Maria \textemdash{} May 2026.}
